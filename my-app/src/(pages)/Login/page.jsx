...
import { Box, Typography, TextField, Button, Divider, IconButton, InputAdornment, Snackbar, Alert } from "@mui/material";
...
const LoginPage = () => {
  ...
  const handleLogin = async () => {
    try {
      ...
      await loginApi({ email, password: pw });
      setSb({ open:true, type:"success", msg:"Login success!" });
      navigate("/home");
    } catch (e) {
      ...
      <Box sx={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center" }}>
        <Box sx={{ width:{ xs:"90%", sm:460 }, p:{ xs:2.5, sm:3.5 } }}>
          ...
          <Box sx={{ p:{ xs:2, sm:3 }, borderRadius:3, backdropFilter:"blur(2px)" }}>
            ...
            <TextField label="Password" type={showPw ? "text" : "password"} fullWidth value={pw} onChange={(e)=>setPw(e.target.value)}
            ...
            <Box sx={{ position:"relative", mb:2.5 }}>
              ...
              <Button fullWidth onClick={handleLogin}
                sx={{ py:1.2, borderRadius:999, bgcolor:"#f57c00", color:"#fff", fontWeight:700, letterSpacing:0.3,
                      boxShadow:"0 6px 16px rgba(0,0,0,.15)", "&:hover":{ bgcolor:"#ff9800" } }}>
                Login
              </Button>
            </Box>
            ...
            <Box sx={{ display:"flex", justifyContent:"space-between", alignItems:"center", mb:2 }}>
              ...
              <Button variant="text" onClick={()=>navigate("/forgetpassword")} sx={{ fontWeight:700, color:"#0d47a1" }}>
                Forget Password ?
              </Button>
            </Box>
            ...
            {/* ปิดปุ่ม Google ชั่วคราวหากยังตั้งค่าไม่เสร็จ */}
          </Box>
        </Box>
      </Box>
